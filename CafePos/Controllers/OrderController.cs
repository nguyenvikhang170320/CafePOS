...
namespace CafePos.Controllers
{
    public class OrderController : Controller
    {
        ...
        public async Task<IActionResult> Create(int orderId, List<int>? selectedToppingIds, decimal discountAmount)
        {
            ViewBag.Toppings = await _context.Toppings
                .Where(x => x.IsActive)
                .ToListAsync();
            ...
            if (order == null)
                return NotFound();
            // CHẶN sửa lại nếu đã checkout xong
            if (order.PaymentStatus == "Paid" || order.OrderStatus == "Completed")
            {
                TempData["Message"] = "Đơn hàng này đã checkout thành công, không thể tạo/chỉnh sửa thêm";
